...
def CreateAnalysisStageWithFlushInstance(cls, global_model, parameters, gt_solver):
    class AnalysisStageWithFlush(cls):
        ...
        def Initialize(self):
            ...
            # We need to set the viscosity ove rthe domain manually
            ...
            # The pressure will be obtained up to a free added constant, so we fix its value in just one node to the ground truth.
            inlet_node_id, inlet_node_pressure = gt_solver.get_inlet_node_and_value()
            inlet_node = self.computing_model_part.GetNode(inlet_node_id)
            inlet_node.SetSolutionStepValue(KratosMultiphysics.PRESSURE, inlet_node_pressure)
            inlet_node.Fix(KratosMultiphysics.PRESSURE)
...
def run_simulation(gt_solver):
    project_parameters_path = "kratos_simulation_tests/first_order_stokes_variable_viscosity_pspg_sd/kratos_files/ProjectParameters.json"
